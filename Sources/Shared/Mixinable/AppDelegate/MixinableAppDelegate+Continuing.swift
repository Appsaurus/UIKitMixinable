//
//  MixinableAppDelegate+Continuing.swift
//  UIKitMixinable
//
...
extension MixinableAppDelegate {
    ...
    open func application(_ application: UIApplication, willContinueUserActivityWithType userActivityType: String) -> Bool {
        var result = false
        for mixin in mixins {
            if mixin.application?(application, willContinueUserActivityWithType: userActivityType) ?? false {
                result = true
            }
        }
        return result
    ...
    open func application(_ application: UIApplication, continue userActivity: NSUserActivity, restorationHandler: @escaping ([UIUserActivityRestoring]?) -> Swift.Void) -> Bool {
        let returns = apply({ (mixin, restorationHandler) -> Bool? in
            mixin.application?(application, continue: userActivity, restorationHandler: restorationHandler)
    ...
    open func application(_ application: UIApplication, didUpdate userActivity: NSUserActivity) {
        for mixin in mixins {
            mixin.application?(application, didUpdate: userActivity)
        }
    }
    ...
    open func application(_ application: UIApplication, didFailToContinueUserActivityWithType userActivityType: String, error: Error) {
        for mixin in mixins {
            mixin.application?(application, didFailToContinueUserActivityWithType: userActivityType, error: error)
        }
    }
}
